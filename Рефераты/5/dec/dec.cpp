...
{
private:
    node* ld, * rd;  // Указатели на левый и правый концы
    int el_left, el_right;  // Переменные для хранения элементов, удаленных с левого и правого концов
public:
    // Прототипы функций
    void POSTROENIE();  // Построение списка
    void VYVOD();  // Вывод списка
    void VSTAV1(int);  // Вставка узла в правый конец
    void VSTAV2(int);  // Вставка узла в левый конец
    int SetElLeft() { return el_left; }  // Получение элемента, удаленного с левого конца
    int SetElRight() { return el_right; }  // Получение элемента, удаленного с правого конца
    void YDALE1();  // Удаление узла с правого конца
    void YDALE2();  // Удаление узла с левого конца
    void OCHISTKA();  // Очистка списка
};

void main()
{
    setlocale(LC_ALL, "Rus");
    Spisok A;  // Создание переменной класса Spisok
    int el;  // Переменная для хранения вводимых пользователем эл. списка
