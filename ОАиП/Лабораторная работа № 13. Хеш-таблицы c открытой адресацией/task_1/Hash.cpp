...
{
	// Инициализация случайных коэффициентов для универсального хеширования
	std::srand(std::time(0));
	int a = 1 + std::rand() % (size - 1);  // a != 0
	int b = std::rand() % size;
	; // Простое число, большее, чем максимальное значение ключа, обычно выбирается заранее
...
bool Object::insert(void* d)
{
	bool b = false;
	if (N != size)
		for (int i = 0, t = getKey(d), 
			j = HashFunction(t, size, 0);
			i != size && !b; 
