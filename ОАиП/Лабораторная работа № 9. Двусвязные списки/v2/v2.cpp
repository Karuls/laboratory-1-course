...
int menu(void)
{
	...
	cout << "4. Поиск" << endl;
	cout << "5. функция подсчёта числа элементов списка с заданным значением x." << endl;
	cout << "0. Выход" << endl;
	cout << endl;
	do
	{
	...
	while (true)
	{
		...
		case 4: {	  char fname[NAME_SIZE];
			...
			find(fname, &head);
			  ...
			  break;
		case 5:  counter = countX(&head); cout <<"Элементов в списке с таким именем: " << counter;
...
int countX(Address** phead) {// функция подсчёта числа элементов списка с заданным значением x
	int sum = 0; char name[25];
	struct Address* t = *phead;
	if (*phead == NULL){
		cout << "Список пуст...";
		return 0;
	...
	if (t != NULL) {
		cout << "Введите имя, поиском которого функиця будет заниматься: ";
		gets_s(name);
	}
	while (t) {
