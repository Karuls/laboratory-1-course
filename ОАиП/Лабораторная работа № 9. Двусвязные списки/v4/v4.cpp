...
int menu(void)
{
	...
	cout << "4. Поиск" << endl;
	cout << "5. функция подсчёта максимального элемента списка" << endl;
	cout << "0. Выход" << endl;
	cout << endl;
	do
	{
	...
	while (true)
	{
		...
		case 4: {	  char fname[NAME_SIZE];
			...
			find(fname, &head);
			  ...
			  break;
		case 5:  counter = findMax(&head); cout << "Максимальный элемент: " << counter;
...
int findMax(Address** phead) {// функция подсчёта максимального числа в списке
	int max;
	struct Address* t = *phead;
	if (*phead == NULL) {
		cout << "Список пуст...";
