...
int menu(void)
{
	...
	cout << "4. Поиск" << endl;
	cout << "5. функция подсчёта минимального элемента списка" << endl;
	cout << "0. Выход" << endl;
	cout << endl;
	do
	{
	...
	while (true)
	{
		...
		case 4: {	  char fname[NAME_SIZE];
			...
			find(fname, &head);
			  ...
			  break;
		case 5:  counter = findMin(&head); cout <<"Минимальный элемент: " << counter;
...
int findMin(Address** phead) {// функция подсчёта минимального числа в списке
	int min;
	struct Address* t = *phead;
	if (*phead == NULL){
		cout << "Список пуст...";
