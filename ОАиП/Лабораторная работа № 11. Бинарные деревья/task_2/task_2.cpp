...
int calculate(Tree* t, int level, int& sum, int& count);     //вычисление среднего арифметического всех элементов дерева
...
void main()
{
	...
	for (;;)
	{
		...
		case 9:  
			int sum = 0, digit = 0;
			calculate(Root, 0, sum, digit);
			cout <<"Среднее арифметическое значений узлов дерева :" << sum / digit<<endl;
		}
	}
}
...
int calculate(Tree* t, int level, int &sum, int& digit) // Вывод дерева
{
	if (t)
	{
		calculate(t->Right, level + 1, sum, digit);    // вывод правого поддерева
			sum += t->key; // изменено: разыменование указателя sum
		digit ++; // изменено: разыменование указателя digit

		calculate(t->Left, level + 1, sum, digit);    // вывод левого поддерева
		return 0;
	}
	return 1;
}
