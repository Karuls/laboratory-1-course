...
int VYVOD_IF_K(Tree* t, int k, int& counter);     //вычисление среднего арифметического всех элементов дерева
...
void main()
{
	...
	for (;;)
	{
		...
		case 9:
			...
			cin >> k;
			VYVOD_IF_K(Root,k,counter);
			cout << "Количество вершин дерева, значение которых равно k: "<< counter << endl;
...
int VYVOD_IF_K(Tree* t, int k, int &counter) // Вывод дерева
{
	if (t)
	{
			VYVOD_IF_K(t->Right, k, counter);    // вывод правого поддерева
		if (t->key == k) {
			cout << t->text<<endl;
			cout << t->key<<endl;
			cout << endl;
		...
		VYVOD_IF_K(t->Left, k, counter);    // вывод левого поддерева
		return 0;
	}
	return 1;
}
