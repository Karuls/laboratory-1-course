...
int countBranchesToKRecursive(Tree* t, int k);
int countBranchesToK(Tree* t, int k);
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	выход\n";
		cout << "9 -	вычисление среднего арифметического всех элементов дерева\n";
		cout << "10 -	подсчет ветвей к заданной вершине\n"; // Новый пункт меню
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		switch (choice)
		...
		case 8:  exit(0);
		case 10: // Новый пункт меню для подсчета ветвей к заданной вершине
			int k_branches;
			cout << "Введите ключ вершины, для которой нужно подсчитать ветви: ";
			cin >> key;
			k_branches = countBranchesToK(Root, key);
			cout << "Количество ветвей к вершине с ключом " << key << " равно " << k_branches << endl;
			break;
		}
	}
}
...
int countBranchesToK(Tree* t, int k)
{
	...
	return countBranchesToKRecursive(t, k);
}
...
int countBranchesToKRecursive(Tree* t, int k)
{
	...
	if (t->key == k) {
		cout << "Ключ найден: " << k << endl;
		return 1;
	}

	if (k < t->key) {
		cout << t->key << " -> ";
		return 1 + countBranchesToKRecursive(t->Left, k);
	}
	else {
		cout << t->key << " -> ";
		return 1 + countBranchesToKRecursive(t->Right, k);
	}
}
