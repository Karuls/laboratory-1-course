...
int count_Right_side(Tree* t);  //Подсчет количества слов
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "8 -	выход\n";
		cout << "8 -	Поиск кол-ва эл правой стороны\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		switch (choice)
		...
		case 7:  delAll(Root); break;
			case 8:  exit(0);
			case 9: Tree * t_Right = Root->Right;
				int s = count_Right_side(t_Right); cout <<"Кол-во элементов в правом под-дереве :"<<  s<<endl; break;
		}
	}
}
...
int count_Right_side(Tree* t) //Подсчет количества слов
{
	if (t)
	{
		count_Right_side(t->Right);
		if (*(t->text) != NULL)
			c++;
		count_Right_side(t->Left);
	}
	return c;
}
