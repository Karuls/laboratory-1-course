...
void sum2(list*);  //функция подсчета кратных 2
void sum_neg(list*);  //функция подсчета отрицательных элементов
...
int main()
{
    ...
    while (choice != 8)
    {
        ...
        case 2:   if (!IsEmpty(first)) // удалить число из списка
        {
            ...
            break;
        case 3:   sum2(first);	// вычисление суммы	кратных 5
            break;
        case 4:   sum_neg(first);	// вычисление суммы	отрицательных элементов
            break;
        case 5:   toFile(first);
            break;
...
void menu(void)  //Вывод меню 
{
    ...
    cout << " 3 - Вычисление суммы чисел кратных 2" << endl;
    cout << " 4 - Вычисление суммы отрицательных элементов " << endl;
...
void sum2(list* p)  // Подсчет суммы чисел кратных 5
{
    int sm = 0;
    if (p == NULL)
        cout << "Список пуст" << endl;
...
void sum_neg(list* p)  // Подсчет суммы отрицательных элементов
{
    int sm = 0;
    if (p == NULL)
        cout << "Список пуст" << endl;
